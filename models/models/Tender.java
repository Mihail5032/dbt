...
public class Tender extends BaseTransactionKey {
    ...
    public RowData toRowDataPst(RowData rawRowData, Schema rawSchema, Schema pstSchema, boolean isCertParty) {
        ...
        // ---- tenderamount: raw varchar → PST decimal(32,2) ----
        Object rawTenderAmount = extractField(rawRowData, rawSchema, "tenderamount");
        if (rawTenderAmount instanceof StringData) {
            BigDecimal amount = parseAmount(((StringData) rawTenderAmount).toString());
            if (amount != null) {
                setPstField(pstRowData, pstSchema, "tenderamount", DecimalData.fromBigDecimal(amount, 32, 2));
            }
        }
